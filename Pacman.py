...
Pacman_image = pygame.transform.rotate(
    pygame.transform.scale(Pacman_image, (PACMAN_WIDTH, PACMAN_HEIGHT)), 0
)
...
PLAYER_LIVES = 0
Lives_Image = pygame.transform.scale(
    pygame.image.load("Sprites\Pacman\Pacman(2).png"),
    (PACMAN_WIDTH - 10, PACMAN_HEIGHT - 10),
)
PLAYER_DEATH = False
EXTRA_LIFE = False
LEVEL_UP = False
...
Pacman_Image_Arrary = [
    pygame.image.load("Sprites\Pacman\Pacman(1).png"),
    pygame.image.load("Sprites\Pacman\Pacman(2).png"),
    pygame.image.load("Sprites\Pacman\Pacman(3).png"),
    pygame.image.load("Sprites\Pacman\Pacman(4).png"),
    pygame.image.load("Sprites\Pacman\Pacman(5).png"),
    pygame.image.load("Sprites\Pacman\Pacman(6).png"),
    pygame.image.load("Sprites\Pacman\Pacman(7).png"),
    pygame.image.load("Sprites\Pacman\Pacman(8).png"),
]
...
def chase_animation(pacman, blinky, clyde, inky, pinky):
    ...
    while i < 4:
        ...
        ghost_body[i] = pygame.transform.scale(
            ghost_body[i],
            (GHOST_SPRITE_HEIGHT * size_mod, GHOST_SPRITE_WIDTH * size_mod),
        )
        ghost_eyes[i] = pygame.transform.scale(
            ghost_eyes[i],
            (GHOST_SPRITE_HEIGHT * size_mod, GHOST_SPRITE_WIDTH * size_mod),
        )
    ...
    Pacman_image = pygame.transform.rotate(
        pygame.transform.scale(
            Pacman_image, (PACMAN_WIDTH * size_mod, PACMAN_HEIGHT * size_mod)
        ),
        0,
    )
...
def draw_ghosts(pacman, blinky, clyde, inky, pinky):
    WINDOW.blit(
        Blinky_Body,
        (blinky.x - GHOST_SPRITE_WIDTH / 2, blinky.y - GHOST_SPRITE_HEIGHT / 2),
    )
    WINDOW.blit(
        Blinky_Eyes,
        (blinky.x - GHOST_SPRITE_WIDTH / 2, blinky.y - GHOST_SPRITE_HEIGHT / 2),
    ...
    WINDOW.blit(
        Inky_Body, (inky.x - GHOST_SPRITE_WIDTH / 2, inky.y - GHOST_SPRITE_HEIGHT / 2)
    )
    WINDOW.blit(
        Inky_Eyes, (inky.x - GHOST_SPRITE_WIDTH / 2, inky.y - GHOST_SPRITE_HEIGHT / 2)
    )
    # pygame.draw.circle(WINDOW,(Inky_Color),(inky.x, inky.y),5)
    # draw clyde and his eyes
    WINDOW.blit(
        Clyde_Body,
        (clyde.x - GHOST_SPRITE_WIDTH / 2, clyde.y - GHOST_SPRITE_HEIGHT / 2),
    )
    WINDOW.blit(
        Clyde_Eyes,
        (clyde.x - GHOST_SPRITE_WIDTH / 2, clyde.y - GHOST_SPRITE_HEIGHT / 2),
    ...
    WINDOW.blit(
        Pinky_Body,
        (pinky.x - GHOST_SPRITE_WIDTH / 2, pinky.y - GHOST_SPRITE_HEIGHT / 2),
    )
    WINDOW.blit(
        Pinky_Eyes,
        (pinky.x - GHOST_SPRITE_WIDTH / 2, pinky.y - GHOST_SPRITE_HEIGHT / 2),
...
def starting_menu(key_released, blinky, clyde, inky, pinky, pacman):
    ...
    if STARTING_MENU == 0:
        ...
        if TIMER % 40 > 15:
            draw_text(
                "PLAY PACMAN", text_font, WHITE, WIDTH / 2, HEIGHT / 2 + offset, True
            )
...
def intro(pacman):
    ...
    if (
        INTRO_SEGMENT
        and INTRO_PLAY == False
        and PLAYER_DEATH == False
        and POWER_UP == False
    ):
        if TIMER % 95 == 0:
        ...
        for j in range(len(dots_left[i])):
            ...
            if dots_left[i][j] == 1:
                pygame.draw.circle(
                    WINDOW,
                    (255, 255, 255),
                    ((num2 * j // 2 + 30) * x_scaler, num1 * i * y_scaler + 83),
                    2,
                )
...
def draw_fruit(pacman):
    ...
    if fruit_available:
        if FRUIT_EATEN == False:
            ...
            if (center_x < pacman.centerx < center_x + 30) and (
                center_y - 8 < pacman.centery < center_y + 8
            ):
...
def board_rest(pacman, blinky, clyde, inky, pinky):
    ...
    if PLAYER_DEATH or LEVEL_UP:
        ...
        Pacman_image = pygame.transform.rotate(
            pygame.transform.scale(Pacman_image, (PACMAN_WIDTH, PACMAN_HEIGHT)),
            rotation,
        )
        if flip:
        ...
        if resume_time > TIMER > resume_time - 240:
            if (
                INTRO_PLAY == False
                and (PLAYER_DEATH or LEVEL_UP) == True
                and TIMER == resume_time - 235
            ):
                if (PLAYER_DEATH and PLAYER_LIVES > 1) or (
                    LEVEL_UP and PLAYER_LIVES > 0
                ):
                    pygame.mixer.Sound.play(Intro, 0)
                    INTRO_PLAY = True
...
def game_over(pacman, blinky, clyde, inky, pinky, key_pressed, key_released):
    ...
    while i < 4:
        ...
        ghost_eyes = [Blinky_Eyes, Clyde_Eyes, Inky_Eyes, Pinky_Eyes]
        ghost_body[i] = pygame.transform.scale(
            ghost_body[i], (GHOST_SPRITE_HEIGHT, GHOST_SPRITE_WIDTH)
        )
        ghost_eyes[i] = pygame.transform.scale(
            ghost_eyes[i], (GHOST_SPRITE_HEIGHT, GHOST_SPRITE_WIDTH)
        )
        Blinky_Body = ghost_body[0]
        Blinky_Eyes = ghost_eyes[0]
        Clyde_Body = ghost_body[1]
...
def input_name(key_released):
    ...
    # current name input
    name = (
        ALHAPHABET[SCORER_NAME[0]]
        + ALHAPHABET[SCORER_NAME[1]]
        + ALHAPHABET[SCORER_NAME[2]]
    )
    ...
    if SUBMITTED == False:
        ...
        pygame.draw.polygon(
            ...
            WHITE,
            (
                (letter_top_left_x, letter_y - 5),
                (letter_top_left_x + (letter_width / 2), letter_y - 10),
                (letter_top_left_x + letter_width, letter_y - 5),
            ),
            0,
        )
        pygame.draw.polygon(
            ...
            WHITE,
            (
                (letter_top_left_x, letter_y + letter_height + 5),
                (letter_top_left_x + (letter_width / 2), letter_y + letter_height + 10),
                (letter_top_left_x + letter_width, letter_y + letter_height + 5),
            ),
            0,
        )
        ...
        if TIMER % 40 > 15:
            ...
            pygame.draw.rect(
                WINDOW,
                BLACK,
                (letter_top_left_x, letter_y, letter_width, letter_height),
                0,
            )
    if SUBMITTED:
    ...
    if game_over_menu_select:
        ...
        while i < list_length:
            ...
            if SCORE > score_entry["score"]:
                draw_text(
                    "You earned a high score", text_font, WHITE, WIDTH / 2, 80, True
                )
                scorer = True
                i = list_length
            i = i + 1
        ...
        if scorer:
            draw_text(
                "Use the Arrow keys to enter your name",
                text_font,
                WHITE,
                WIDTH / 2,
                100,
        ...
        for j in range(len(Active_area[i])):
            ...
            if Active_area[i][j] == 1:
                pygame.draw.circle(
                    ...
                    (255, 0, 0),
                    (
                        ((num2 * j // 2) * x_scaler) + x_offset,
                        ((num1 * i * y_scaler) + y_offset),
                    ),
                    3,
                )
        ...
        for j in range(len(Active_area[i])):
            ...
            if Active_area[i][j] == 1:
                pygame.draw.circle(
                    ...
                    (255, 0, 0),
                    (
                        WIDTH - ((num2 * j // 2) * x_scaler) - x_offset,
                        ((num1 * i * y_scaler) + y_offset),
                    ),
                    3,
                )
        ...
        if d_x != PACMAN_DELTA_X and d_y != PACMAN_DELTA_Y:
            ...
            if d_y != 0:
                for i in range(len(column_center)):
                    if sprite.centerx > (
                        column_center[i] - x_offset
                    ) and sprite.centerx < (column_center[i] + x_offset):
                        return True
                return False
            if d_x != 0:
                for i in range(len(row_center)):
                    if sprite.centery > (
                        row_center[i] - y_offset
                    ) and sprite.centery < (row_center[i] + y_offset):
                        return True
                return False
        return True
...
def pacman_center(sprite, d_x, d_y):
    ...
    if PACMAN_DELTA_X == 0:
        for i in range(len(column_center)):
            if sprite.centerx > (column_center[i] - x_offset) and sprite.centerx < (
                column_center[i] + x_offset
            ):
    ...
    if PACMAN_DELTA_Y == 0:
        for i in range(len(row_center)):
            if sprite.centery > (row_center[i] - y_offset) and sprite.centery < (
                row_center[i] + y_offset
            ):
        ...
        if (
            ...
            if SCATTER == False:
                ...
                if Eaten_Blinky == False:
                    ...
                    if hunt == 4:
                        (
                            ...
                            BLINKY_DELTA_Y,
                            node,
                        ) = ghost_node_vectoring(ghost, d_x, d_y, 0, 0, node, fright)
                ...
                if Eaten_Inky == False:
                    ...
                    hunt = huntable_zone(d_x, d_y, ghost)
                    if hunt == 4:
                        ghost, INKY_DELTA_X, INKY_DELTA_Y, node = ghost_node_vectoring(
                            ghost, d_x, d_y, 0, 0, node, fright
                        )
...
def blinky_movement(blinky, pacman):
    ...
    if eaten == False:
        Eaten_Blinky, BLINKY_FRIGHTENED = pacman_contact(
            sprite, pacman, eaten, frightened
        )
    else:
        BLINKY_FRIGHTENED = False
    ...
    if BLINKY_HOUSE:
        if (WIDTH / 2 - 4) < blinky.x < (WIDTH / 2 + 4):
            ...
            d_y = 0
            if Eaten_Blinky:
                Eaten_Blinky, BLINKY_FRIGHTENED, d_x, d_y = enter_ghost_house(
                    blinky, d_x, d_y
                )
        if Eaten_Blinky == False:
    ...
    Blinky_Body = pygame.transform.scale(
        Blinky_Body, (GHOST_SPRITE_HEIGHT, GHOST_SPRITE_WIDTH)
    )
    Blinky_Eyes = pygame.transform.scale(
        Blinky_Eyes, (GHOST_SPRITE_HEIGHT, GHOST_SPRITE_WIDTH)
    )
...
def blinky_AI(blinky, pacman, d_x, d_y, node_count):
    ...
    ghost = blinky
    frightened = BLINKY_FRIGHTENED
    ghost, d_x, d_y, node = ghost_node_vectoring(
        ghost, d_x, d_y, x, y, node, frightened
    )
    Node_Blinky = True
    return d_x, d_y, Node_Blinky
...
def clyde_movement(clyde, pacman):
    ...
    if eaten == False:
        Eaten_Clyde, CLYDE_FRIGHTENED = pacman_contact(
            sprite, pacman, eaten, frightened
        )
    else:
        CLYDE_FRIGHTENED = False
    ...
    # scale up eye and body image
    Clyde_Body = pygame.transform.scale(
        Clyde_Body, (GHOST_SPRITE_HEIGHT, GHOST_SPRITE_WIDTH)
    )
    Clyde_Eyes = pygame.transform.scale(
        Clyde_Eyes, (GHOST_SPRITE_HEIGHT, GHOST_SPRITE_WIDTH)
    )
...
def pinky_movement(pinky, pacman):
    ...
    if eaten == False:
        Eaten_Pinky, PINKY_FRIGHTENED = pacman_contact(
            sprite, pacman, eaten, frightened
        )
    else:
        PINKY_FRIGHTENED = False
    ...
    # scale up eye and body image
    Pinky_Body = pygame.transform.scale(
        Pinky_Body, (GHOST_SPRITE_HEIGHT, GHOST_SPRITE_WIDTH)
    )
    Pinky_Eyes = pygame.transform.scale(
        Pinky_Eyes, (GHOST_SPRITE_HEIGHT, GHOST_SPRITE_WIDTH)
    )
...
def inky_movement(inky, pacman, blinky):
    ...
    if INKY_HOUSE and ((TIMER - ROUND_START) / 60 > 1):
        ...
        if (WIDTH / 2 - 4) < inky.x < (WIDTH / 2 + 4):
            ...
            d_y = 0
            if Eaten_Inky:
                Eaten_Inky, INKY_FRIGHTENED, d_x, d_y = enter_ghost_house(
                    inky, d_x, d_y
                )
    ...
    # scale up eye and body image
    Inky_Body = pygame.transform.scale(
        Inky_Body, (GHOST_SPRITE_HEIGHT, GHOST_SPRITE_WIDTH)
    )
    Inky_Eyes = pygame.transform.scale(
        Inky_Eyes, (GHOST_SPRITE_HEIGHT, GHOST_SPRITE_WIDTH)
    )
...
def main():
    pacman = pygame.Rect(
        (PACMAN_STARTING_X, PACMAN_STARTING_Y, PACMAN_WIDTH, PACMAN_HEIGHT)
    )
